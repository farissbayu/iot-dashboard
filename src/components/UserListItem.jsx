export default function UserListItem({ user }) {
  return (
    <tr className="shadow-lg bg-white hover:bg-gray-50">
      <td className="p-4 max-w-8">{user.id}</td>
      ...
      <td className="p-4 max-w-16">{user.isAdmin ? "admin" : "user"}</td>
      <td className="p-4 max-w-16">
        <button
          className="bg-buttonRed p-1 rounded-lg opacity-75 hover:opacity-100"
          onClick={() => {
